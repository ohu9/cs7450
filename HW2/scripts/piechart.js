...
// MARGIN CONVENTIONS
const pieMargin = { top: 20, right: 20, bottom: 70, left: 70 };
const pieOuterWidth = 1000;
const pieOuterHeight = 520;
const pieWidth = pieOuterWidth - pieMargin.left - pieMargin.right;
const pieHeight = pieOuterHeight - pieMargin.top - pieMargin.bottom;
...
d3.csv("atl_weather_20to22.csv", cleanWeatherData).then(data => {
    ...
    pieG.append("text")
        ...
        .attr("x", pieWidth / 2)
        .attr("y", pieHeight + pieMargin.bottom - 20)
        .text("Date");
    ...
    pieG.append("text")
        ...
        .attr("x", -pieHeight / 2)
        .attr("y", -pieMargin.left + 30)
        .text("Temperature");

    // interaction — linked tooltips for both lines
    const focusG = pieG.append("g")
        .style("display", "none");
